...
ByVal limpiar As Boolean, ByVal t1 As Single, ByVal t2 As Single, ByVal Di As Integer)
    ...
    ByVal pb As Object, ByVal rgB(,) As Double, ByVal colGR As Color, ByVal colBk As Color, _
    ByVal limpiar As Boolean, Optional ByVal MostrarConjugado As Boolean = True, Optional ByVal SuaV As Int16 = 0)
        ...
        Dim valor As Double = 1
        If limpiar = True Then
            gr.Clear(colBk) 'Para limpiar el picturebox
        ...
        For i = 1 To final - 2  'el gráfico
            Try
                gr.DrawLine(fn, CSng(i), CSng(rg(i) * valor), CSng(i + 1), CSng(rg(i + 1) * valor))
            Catch ex As Exception
                If Err.Description = "Error de desbordamiento." Then
                    Err.Clear()
                    Form1.st.Text = "Error de desbordamiento."
                    i = 0
                    valor = valor * 10
                Else
                    Err.Clear()
    ...
    Public Shared Sub DrawTransflog(ByVal gr As Graphics, _
        ByVal pb As Object, ByVal rgB(,) As Double, ByVal colGR As Color, ByVal colBk As Color, _
        ByVal limpiar As Boolean, Optional ByVal LogHorz As Boolean = True, _
        Optional ByVal MostrarConjugado As Boolean = True, Optional ByVal SuaV As Int16 = 0)
        ...
        If limpiar = True Then
            gr.Clear(colBk) 'Para limpiar el picturebox
            If LogHorz = True Then
                MapGraphicsWindow(gr, 0, ymin, CSng(Math.Log10(final)), ymax, 0, pb.Height - 1, pb.Width - 1, 0)
            Else
        ...
        Dim fz As New Pen(Color.WhiteSmoke, 0.005)
        Dim ln As New Pen(Color.Yellow, 0.005)
        Dim lm As New Pen(Color.DarkGray, 0.0)

        For i = 0.1 To 0.9 Step 0.1
            gr.DrawLine(lm, CSng(0), CSng(Math.Log10(i) * 1000000), CSng(rg.Length - 1), CSng(Math.Log10(i) * 1000000))
        Next
        If LogHorz = True Then
            For i = 1 To final - 1 Step final / 10
                gr.DrawLine(lm, CSng(Math.Log10(i)), ymin, CSng(Math.Log10(i)), ymax)
            Next
        End If
        gr.DrawLine(fz, 0, 0, rg.Length - 1, 0) 'linea de cero
        ...
        If Transf.handleDAMPING = True Then
            If Transf.T1 <> 0 Then
                If LogHorz = True Then
                    gr.DrawLine(ln, CSng(Math.Log10(Transf.T1 / (SuaV * 2 + 1))), CSng(ymin), CSng(Math.Log10(Transf.T1 / (SuaV * 2 + 1))), CSng(ymax))
                Else
            ...
            If Transf.T2 <> 0 Then
                If LogHorz = True Then
                    gr.DrawLine(ln, CSng(Math.Log10(Transf.T2 / (SuaV * 2 + 1))), CSng(ymin), CSng(Math.Log10(Transf.T2 / (SuaV * 2 + 1))), CSng(ymax))
                Else
        ...
        For i = 1 To final - 1 Step 3 'el gráfico
            Try
                If LogHorz = True Then
                    gr.DrawBezier(fn, CSng(Math.Log10(i)), CSng(rg(i)), CSng(Math.Log10(i + 1)), CSng(rg(i + 1)), CSng(Math.Log10(i + 2)), CSng(rg(i + 2)), CSng(Math.Log10(i + 3)), CSng(rg(i + 3)))
                Else
