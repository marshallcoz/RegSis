...
#Region "Declaraciones"
    ...
    Dim Tam_Orig As Single 'tamaño original del registro (se usa en antiFFT)
    Public Shared MaG() As Double 'Módulo del espectro
    ...
    Private Sub Ventana_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.TextChanged
        Select Case Me.Text.Substring(0, 4)
            ...
            Case "[CP]"            '-------------------------------------------------copiar-----------------------
                ...
                'copiar la señal al portapapeles
                Dim t As String = ""
                Dim z As Single
                For z = 0 To CInt(rg.Length / 2) - 1
                    If rg(z, 0) < 0 Then
            ...
            Case "[FT]"           '---------------------------------------fast-fourier-trasnform-------------------
                Tam_Orig = rg.Length / 2 - 1 'Tamaño original del registro
                rgF = TransformadaRápidaDeFourier(rg, Tam_Orig)
                MaG = magnitud(rgF)
            ...
            Case "[Ft]"           '------------------------------------filtrar---------------------------------------------
                ...
                For i = 0 To rgF.Length / 2 - 1
                    rgF(i, 0) = rgF(i, 0) / (rgF.Length / 2) 'se usa para escalar el registro, el tamaño total del espectro
                    rgF(i, 1) = rgF(i, 1) / (rgF.Length / 2)
                Next

                TRFwndw.Dispose()
                rg = AntiTransformadaRápidaDeFourier(rgF)
...
#Region "Impresión"
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles printdocument1.PrintPage
        ...
        If Me.Text.StartsWith("[Pg]") Then
            'encontrar el máximo de los dos acelerogramas el del registro basa ya está es max_amp del caso anterior
        ...
        j = 0
        If Me.Text.StartsWith("[Pg]") Then
            Tam_Orig = Form1.clonrg.Length / 2 - 1 'Tamaño original del registro
            Dim rgf2(,) As Double = TransformadaRápidaDeFourier(Form1.clonrg, Tam_Orig)
            mag2 = magnitud(rgf2)
        ...
        For i = 0 To 12 'El eje vertical 
            ...
            fontchica, Brushes.Gray, 75, 100 + CInt(250 * (i / 12))) 'Los números
        Next
        ...
        e.Graphics.DrawString("Duración= " & Format(rg((rg.Length / 2) - 1, 1) / 1, "#.0000") & " segundos", font, Brushes.Black, 100, 460)
        ...
        For i = 1 To (Form1.SPS / 10)
            e.Graphics.DrawLine(grisborde, 140 + CInt(840 * (i / (Form1.SPS / 10))), 750, 140 + CInt(840 * (i / (Form1.SPS / 10))), 755) 'Las rayitas de división a las mitades
        Next
        For i = 1 To (Form1.SPS / 5)
            e.Graphics.DrawLine(grisborde, 140 + CInt(840 * (i / (Form1.SPS / 5))), 750, 140 + CInt(840 * (i / (Form1.SPS / 5))), 753) 'Las rayitas de división a los cuartos
        Next
        For i = 1 To (Form1.SPS / 20) 'El eje horizontal. Dependiendo del SPS
            ...
            e.Graphics.DrawLine(rojo, 140 + CInt(840 * (i / (Form1.SPS / 20))), 750, 140 + CInt(840 * (i / (Form1.SPS / 20))), 757) 'Las rayitas de división rojas en el eje horizontal
        ...
        For i = 0 To 12  'El eje vertical 
            ...
            fontchica, Brushes.Gray, 75, 750 - CInt(250 * (i / 12))) 'Los números
        Next
    ...
    100)
        ...
        If Me.Text.StartsWith("[Pg]") Then
            DrawOnPictureBox(e.Graphics, PIC, rg, colortrazo, False) 'cuando se dibuja el gráfico 
            If Form1.clonrg.Length > rg.Length Then
                'se va a salir de la gráfica, así que le cortamos un cachito
                Dim clonAUX(,) As Double
                ReDim clonAUX(rg.Length / 2, 1)
                Array.Copy(Form1.clonrg, clonAUX, CInt(rg.Length - 1))
                DrawOnPictureBox(e.Graphics, PIC, clonAUX, colorclon, False) 'cuando se dibuja el gráfico el clon
            Else
                DrawOnPictureBox(e.Graphics, PIC, Form1.clonrg, colorclon, False) 'cuando se dibuja el gráfico el clon
            End If
        Else
            DrawOnPictureBox(e.Graphics, PIC, rg, colortrazo, False) 'cuando se dibuja el gráfico 
        End If
        ...
        100000 * (Math.Abs(max_amp - min_amp))) 'El rectángulo del borde del gráfico
...
#Region "Menu emergente"
    ...
    Private Sub CopiarSeñal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuItem10.Click
        Me.Text = "[CP]" & Me.Text
    End Sub

    Private Sub PegarSeñal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MenuItem12.Click
        Me.Text = "[Pg]" & Me.Text
    End Sub
...
#End Region
    ...
    Sub MMM() 'reportar el máximo, el mínimo, la distancia entre estos y la media de la señal
